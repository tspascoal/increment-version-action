    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (incrementTypeIdx < 0) {
                core.setFailed(`increment type needs to be one of  ${increments.join(',')}`);
                return;
            }
            const components = version.split('-');
            const versionParts = components[0].split('.');
            if (versionParts.length < incrementTypeIdx + 1) {
                core.setFailed(`version ${version} doesn't contain ${incrementType} component`);
                return;
            }
            versionParts[incrementTypeIdx] = (parseInt(versionParts[incrementTypeIdx]) + increment).toString();
            core.setOutput('version', versionParts.join('.') + (components.length > 1 ? `-${components[1]}` : ''));
        }
        catch (error) {
